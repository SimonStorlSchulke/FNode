...
public partial class FNodeMoveFile : FNode
{
    public FNodeMoveFile() {
        TooltipText = "Moves a File to the given folder and creates a Folder if the path doesn't exist";
        category = "File";
        FNode.IdxReset();
        inputs = new System.Collections.Generic.Dictionary<string, FInput>() {
            {"File", new FInputFile(this)},
            {"To Path3D", new FInputString(this)},
    ...
    public override void ExecutiveMethod()
    {
        ...
        string path = inputs["To Path3D"].Get<string>();
